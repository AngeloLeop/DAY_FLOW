...
(function (root) {
  ...
  DF.conflicts = {
    detect(items) { const sorted = [...items].sort((a, b) => DF.dates.timeToMinutes(a.startTime) - DF.dates.timeToMinutes(b.startTime)); const found = []; for (let i = 1; i < sorted.length; i++) if (DF.dates.overlaps(sorted[i - 1], sorted[i])) found.push([sorted[i - 1], sorted[i]]); return found; },
